// ─── OrgDistributionChart — Distribución de ventas por zona (donut) ───────────
    ...
    return (
        <div className="bg-gray-900 border border-white/[0.10] px-4 py-3 shadow-xl">
            ...
            <div className="text-[11px] text-gray-400 space-y-0.5">
                ...
                <p>Proporción: <span className="text-white font-semibold tabular-nums">{datos.porcentaje}%</span></p>
            </div>
        </div>
    );
};

const OrgDistributionChart = ({ datos, cargando }) => {
    ...
    const datosConPorcentaje = datos.map(d => ({
        ...d,
        porcentaje: totalTickets > 0 ? Math.round((d.tickets / totalTickets) * 1000) / 10 : 0,
    }));

    return (
        <div className="bg-white/[0.02] border border-white/[0.06] p-5" role="region"
            aria-label="Distribución de ventas por zona">
            <div className="flex items-center gap-3 mb-5">
                <div className="w-0.5 h-5 bg-emerald-500/70 rounded-full" />
                <div>
                    <h2 className="text-sm font-bold text-white">Ventas por Zona</h2>
                    <p className="text-xs text-gray-400 mt-0.5">Distribución por categoría de entrada</p>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
                <div className="w-full sm:w-1/2">
                    <ResponsiveContainer width="100%" height={220}>
                        <PieChart tabIndex={-1}>
                            <Pie data={datosConPorcentaje} cx="50%" cy="50%" innerRadius={60} outerRadius={90}
                                dataKey="tickets" stroke="rgba(3,7,18,0.6)" strokeWidth={2}>
                        ...
                        <div key={item.categoria} className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-gray-300 font-medium truncate">{item.categoria}</span>
                                    <span className="text-xs text-white font-semibold tabular-nums ml-2">{item.porcentaje}%</span>
                                </div>
                                <div className="h-1 bg-white/[0.06] rounded-full mt-1 overflow-hidden">
                                    <div className="h-full rounded-full transition-all duration-500"
                                        style={{ width: `${item.porcentaje}%`, backgroundColor: COLORES[index % COLORES.length] }} />
                                </div>
                            </div>
                        </div>
                    ))}
